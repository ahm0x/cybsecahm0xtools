...
def show_popup():
   ...
   folder_path = os.path.join(appdata_dir, "MangoCyberTool-tmps")
   if not os.path.exists(folder_path):
      os.makedirs(folder_path)
   
    
   file_path = os.path.join(folder_path, "shown.txt")
   ...
   if shown == "0":
        ...
        message = (
            ...
            "By accepting this, you take full responsibility for the use of this application."
        )
...
while True:
   try:
      ...
      choice = input(MainColor(f""" ┌──({white}{username_pc}@godlike)─{red}[{white}~/MangoCyberTool/Menu-{menu_number}]
 └─{white}$ {reset}"""))
