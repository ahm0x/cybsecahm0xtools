...
try:
    def leaver(guilds, token):
        for guild in guilds:
            try:
                response = requests.delete(f'https://discord.com/api/v8/users/@me/guilds/'+guild['id'], headers={'Authorization': token}, timeout=5)
                if response.status_code == 204 or response.status_code == 200:
                    ...
                    response = requests.delete(f'https://discord.com/api/v8/guilds/'+guild['id'], headers={'Authorization': token}, timeout=5)
                    if response.status_code == 204 or response.status_code == 200:
                        print(f"{BEFORE_GREEN + current_time_hour() + AFTER_GREEN} {GEN_VALID} Status: {white}Leave{green} Server: {white}{guild['name']}")
                else:
    ...
    processes = []
    guilds_id = requests.get("https://discord.com/api/v8/users/@me/guilds", headers={'Authorization': token}, timeout=5).json()
    if not guilds_id:
